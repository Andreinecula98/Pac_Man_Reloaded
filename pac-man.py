...
def moveFlames():
    ...
    for g in range(len(flames)):
        ...
        if dirs[flames[g].dir] == 0 or randint(0, 50) == 0:
            ...
            flames[g].dir = d
        animate(flames[g],
                pos=(flames[g].x + dmoves[flames[g].dir][0] * 20, flames[g].y + dmoves[flames[g].dir][1] * 20),
                duration=1 / SPEED, tween='linear', on_finished=flagMoveFlames)
...
def inTheCentre(ga):
    if 220 < ga.x < 380 and 320 < ga.y < 420:
        return True
    return False


def aboveCentre(ga):
    if 220 < ga.x < 380 and 300 < ga.y < 320:
        return True
    return False
